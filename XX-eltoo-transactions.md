...
* version: 2
* locktime: 500000000+k+1, where `k` equals the latest committed update transaction version
* txin count: 1
...
There are `N` copies of this output, one for each channel partner and their associated `localpubkey` derived for that particular settlement transaction.

Since these outputs can be immediately spent, they can be used for CPFP fee bumping as required to achieve confirmation.

FIXME it's better if the owner is able to set the inner pubkey to a NUMS and/or attach a merkle root to their output pubkey, maybe while
keeping derivation compatible across clients? You really don't need the mixing of public and private info with symmetrical state, do you?
...
FIXME figure out dust limit stuff
